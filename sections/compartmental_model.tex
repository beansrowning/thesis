...
\begin{figure}[htbp]
  ...
  \coordinate[below=of I2, xshift=-0.5cm] (in4);
  \coordinate[below=of S2] (out1);
  \coordinate[below=of E2] (out2);
  \coordinate[below=of I2, xshift=0.5cm] (out3);
  ...
  \path[->]
            ...
            ([xshift=0.5cm]I2.south)   edge node {$\Omega$}    (out3)
            (R2)   edge node {$\Omega$}                        (out4)
            % Case Introductions
            (in3)  edge node {$f_{t}$}                      (I1)
            (in4) edge node {$f_{t}$}          ([xshift=-0.5cm]I2);
  \end{tikzpicture}
...
\end{figure}
  Where:
   ...
   $f_{t} = $ New case introduction rate
